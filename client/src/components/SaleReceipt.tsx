...
const SaleReceipt = forwardRef<HTMLDivElement, SaleReceiptProps>(({ sale }, ref) => {
  ...
  // Removido paymentMethodLabels - usando formatPaymentMethod do formatters.ts
  ...
  return (
    <div ref={ref} className="receipt-container">
      <div className="receipt-content">
        ...
        {/* Informações da Venda - Grid Compacto */}
        <div className="sale-info">
          <div className="info-grid">
            <div><strong>Venda Nº:</strong> {sale.saleNumber}</div>
            <div><strong>Data:</strong> {formatDate(sale.createdAt)}</div>
            <div><strong>Vendedor:</strong> {sale.sellerName}</div>
            <div><strong>Status:</strong> <span className={`status-${sale.status}`}>
              {sale.status === 'concluida' ? 'Concluída' : sale.status === 'cancelada' ? 'Cancelada' : 'Pendente'}
            </span></div>
          </div>
          {sale.clientName && (
            <div className="client-info">
              <strong>Cliente:</strong> {sale.clientName} {sale.clientDocument ? `- ${sale.clientDocument}` : ''}
            </div>
          )}
        </div>
        ...
        <div className="totals">
          ...
          {sale.discount > 0 && (
            <div className="total-row">
              <span>Desconto:</span>
              <span className="discount">- R$ {formatCurrency(sale.discount)}</span>
            </div>
          )}
          {sale.deliveryFee && sale.deliveryFee > 0 && (
            <div className="total-row">
              <span>Frete:</span>
              <span>R$ {formatCurrency(sale.deliveryFee)}</span>
            </div>
          )}
        ...
        {/* Entrega - Integrado com Data/Hora */}
        {sale.deliveryType && sale.deliveryType !== 'balcao' && (
          <>
            <div className="divider"></div>
            <div className="delivery-info">
              <div className="delivery-grid">
                <div><strong>Entrega:</strong> {sale.deliveryType === 'entrega' ? 'Entrega' : 'Retirada no Depósito'}</div>
                {(sale as any).deliveryDate && (
                  <div><strong>Data:</strong> {typeof (sale as any).deliveryDate === 'string' ? (sale as any).deliveryDate : formatDate((sale as any).deliveryDate)} {(sale as any).deliveryTime && `às ${(sale as any).deliveryTime}`}</div>
                )}
              </div>
              {sale.deliveryAddress && (
                <div className="delivery-address">
                  <strong>Endereço:</strong> {sale.deliveryAddress}
                </div>
              )}
              {sale.deliveryStatus === 'entregue' && (sale as any).deliveredByName && (
                <div className="delivery-completed">
                  <strong>{sale.deliveryType === 'deposito' ? 'Retirado por:' : 'Entregue por:'}</strong> {(sale as any).deliveredByName} em {formatDate((sale as any).deliveredAt)}
                </div>
              )}
            </div>
          </>
        ...
        {sale.status === 'cancelada' && sale.cancellation && (
          <>
            <div className="divider"></div>
            <div className="cancellation">
              <h3>⚠️ VENDA CANCELADA</h3>
              <p><strong>Cancelado por:</strong> {sale.cancellation.cancelledByName}</p>
              <p><strong>Data:</strong> {formatDate(sale.cancellation.cancelledAt)}</p>
              <p><strong>Motivo:</strong> {sale.cancellation.reason}</p>
            </div>
          </>
        )}
