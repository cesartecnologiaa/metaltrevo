...
export default function AccountsReceivable() {
    ...
    try {
      ...
      setLastPayment({
        ...
        installment: {
          ...
          originalAmount: getOriginalInstallmentAmount(selectedPayment.installment),
          paidAmount: roundMoney(amount),
          amount: getOriginalInstallmentAmount(selectedPayment.installment),
          paidAt: Timestamp.now(),
          paidByUserName: userData.name,
        } as Installment,
        paidAmount: roundMoney(amount),
  ...
  const getOriginalInstallmentAmount = (installment: any) =>
    roundMoney(Number(installment?.originalAmount ?? installment?.amount ?? 0));

  const getPaidInstallmentAmount = (installment: any, fallback = 0) =>
    roundMoney(Number(installment?.paidAmount ?? fallback ?? installment?.amount ?? 0));
  ...
  const currentInstallmentAmount = getOriginalInstallmentAmount(selectedPayment?.installment);
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <p className="text-white font-medium">{formatCurrency(getOriginalInstallmentAmount(installment))}</p>
                            </div>
                            <div>
                            ...
                            {installment.paidAt && (
                              <>
                                ...
                                <div>
                                  ...
                                  <p className="text-white font-medium">{(installment as any).paidByUserName || (installment as any).paidByName || '-'}</p>
                                </div>
                              </>
                            )}
                          </div>
                        ...
                        <div className="flex gap-2">
                          {installment.status === 'cancelada' ? (
                            ...
                            <div className="text-right">
                              <p className="text-emerald-300 text-sm font-semibold">Pagamento registrado</p>
                              <p className="text-white/60 text-xs">
                                Pago: {formatCurrency(getPaidInstallmentAmount(installment))}
                              </p>
                            </div>
                          ) : (
                            <Button
            ...
            <DialogContent className="backdrop-blur-2xl bg-gradient-to-br from-blue-900/95 to-cyan-900/95 border-white/20">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    value={String(getOriginalInstallmentAmount(selectedPayment.installment).toFixed(2))}
                    readOnly
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                    <Button type="button" variant="outline" onClick={() => setPaymentAmount(String(getOriginalInstallmentAmount(selectedPayment.installment).toFixed(2)))} className="bg-white/5 border-white/20 text-white hover:bg-white/10">Quitar</Button>
                    <Button type="button" variant="outline" onClick={() => setPaymentAmount(String(Number((getOriginalInstallmentAmount(selectedPayment.installment) / 2).toFixed(2))))} className="bg-white/5 border-white/20 text-white hover:bg-white/10">50%</Button>
                    <Button type="button" variant="outline" onClick={() => setPaymentAmount(String(Number((getOriginalInstallmentAmount(selectedPayment.installment) * 0.25).toFixed(2))))} className="bg-white/5 border-white/20 text-white hover:bg-white/10">25%</Button>
                    ...
                    {isPartialPayment && (
                      <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
                        ...
                        <p className="text-yellow-200 text-sm mt-1">
                          Restante de {formatCurrency(remainingAmount)} será lançado na próxima cobrança em aberto ou gerará uma nova cobrança se esta for a última parcela.
                        </p>
                      </div>
                    )}
